...
const int NUM_OF_BOX_ELEMENTS = 7; // 每个box包含的元素数量：left, top, right, bottom, confidence, class, keep_flag  随后再附加所有维度的权重
...
static __global__ void decode_nfb_kernel(float* predict, int box_count, int class_count, float confidence_threshold, float* d2s_matrix, int mask_dim, float* temparray, int ret_limit){
    ...
    // 4,将结果写入temparray
    float* dest = temparray + 1 + index * (NUM_OF_BOX_ELEMENTS + mask_dim);
    *dest++ = left;         // left
    *dest++ = top;          // top
    *dest++ = right;            // rihgt
    *dest++ = bottom;            // bottom
...
static void decode_boxes(float* predict, int box_count, int class_count, float confidence_threshold, float nms_threshold, float* d2s_matrix, float* temparray, int ret_limit, int mask_dim, cudaStream_t& stream){
    int threads = std::min(256, box_count);
    int blocks = (box_count + threads - 1) / threads;
    decode_nfb_kernel<<<blocks, threads, 0, stream>>>(predict, box_count, class_count, confidence_threshold, d2s_matrix, mask_dim, temparray, ret_limit);
    CHECK(cudaStreamSynchronize(stream));

    threads = std::min(256, ret_limit);
    blocks = (ret_limit + threads - 1) / threads;
    nms_kernel<<<blocks, threads, 0, stream>>>(temparray, NUM_OF_BOX_ELEMENTS + mask_dim, ret_limit, nms_threshold);
    CHECK(cudaStreamSynchronize(stream));
}
        ...
        cudaStream_t& stream)
{
    ...
    decode_mask_kernel<<<grid, block, 0, stream>>>(mask_predict, mask_width, mask_height, mask_dim, mask_threshold, left, top, mask_weights, mask_out, out_width, out_height);
}
...
void postprocess_seg_by_cuda(
        ...
        cudaStream_t& stream)
{
    YOLO::CudaMemory<float> temparray;
    temparray.malloc((NUM_OF_BOX_ELEMENTS + mask_dim) * ret_limit + 1);
    // 解析检测头，将合适的box写入temparray，并进行nms操作。此时可以从temparray中提取所有有效的bbox
    decode_boxes(predict, box_count, class_count, confidence_threshold, nms_threshold, d2s_matrix, temparray.gpu(), ret_limit, mask_dim, stream);
    // 将temparray从device拷贝到host，准备尽心下一步mask头解析
    temparray.memcpy_to_cpu_sync(stream);
    ...
    for(int i = 0; i < count; ++i){
        float* cur_item = temparray.cpu() + 1 + i * (NUM_OF_BOX_ELEMENTS + mask_dim);
        if((int)(cur_item[6]) == 0){
            // 这个box被抑制了，不需要输出
            continue;
        }
        YOLO::SegmentationResult box;
        box.left = cur_item[0];
        box.top = cur_item[1];
        ...
        if(mask_out_w > 0 && mask_out_h > 0){
            ...
            CHECK(cudaMallocHost((void**)&box_buf.data, mask_out_w * mask_out_h));
            CHECK(cudaMalloc((void**)&box_buf.data_device, mask_out_w * mask_out_h));
            decode_mask(mask_predict, mask_w, mask_h, mask_dim, mask_threshold,
                    left_t * scale_x, top_t * scale_y, cur_item + 7, box_buf.data_device, mask_out_w, mask_out_h, stream);
            CHECK(cudaMemcpyAsync(box_buf.data, box_buf.data_device, mask_out_w * mask_out_h, cudaMemcpyDeviceToHost, stream));
    ...
    for(int i = 0; i < (int)temp_buffer.size(); ++i){
        ...
        if(box.data != nullptr){
            ...
            CHECK(cudaFreeHost(box.data));
            CHECK(cudaFree(box.data_device));
        }
    }
