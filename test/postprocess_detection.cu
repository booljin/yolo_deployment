...
static __global__ void decode_nfb_kernel(float* predict, int box_count, int class_count, float confidence_threshold, float* d2s_matrix, float* temparray, int ret_limit){
    ...
    // 4,将结果写入temparray
    float* dest = temparray + 1 + index * (NUM_OF_BOX_ELEMENTS);
    *dest++ = left;         // left
    *dest++ = top;          // top
    *dest++ = right;            // rihgt
    *dest++ = bottom;            // bottom
...
static void decode_boxes(float* predict, int box_count, int class_count, float confidence_threshold, float nms_threshold, float* d2s_matrix, float* temparray, int ret_limit, cudaStream_t& stream){
    int threads = std::min(256, box_count);
    int blocks = (box_count + threads - 1) / threads;
    decode_nfb_kernel<<<blocks, threads, 0, stream>>>(predict, box_count, class_count, confidence_threshold, d2s_matrix, temparray, ret_limit);
    CHECK(cudaStreamSynchronize(stream));

    threads = std::min(256, ret_limit);
    blocks = (ret_limit + threads - 1) / threads;
    nms_kernel<<<blocks, threads, 0, stream>>>(temparray, NUM_OF_BOX_ELEMENTS, ret_limit, nms_threshold);
    CHECK(cudaStreamSynchronize(stream));
}
        ...
        cudaStream_t& stream)
{
    ...
    temparray.malloc(NUM_OF_BOX_ELEMENTS * ret_limit + 1);
    // 解析检测头，将合适的box写入temparray，并进行nms操作。此时可以从temparray中提取所有有效的bbox
    decode_boxes(predict, box_count, class_count, confidence_threshold, nms_threshold, d2s_matrix, temparray.gpu(), ret_limit, stream);
    ...
    for(int i = 0; i < count; ++i){
        float* cur_item = temparray.cpu() + 1 + i * (NUM_OF_BOX_ELEMENTS);
        if((int)(cur_item[6]) == 0){
            // 这个box被抑制了，不需要输出
            continue;
        }
        YOLO::DetectionResult result;
