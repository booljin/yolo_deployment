...
*/
static __global__ void preprocess_kernel_t(uint8_t* img_buffer_device, float* input_buff_device, int src_w, int src_h, int dst_w, int dst_h, float* d2s_matrix, int edge){
...
void preprocess_by_cuda_t(cv::Mat& img, float* device_input, int dst_h, int dst_w, float* d2s_matrix, cudaStream_t& stream){
    // 将图片上传至显存
    //CHECK(cudaMalloc((void**)device_input, input_c * input_h * input_w));
    uint8_t* img_buffer_host = nullptr;
    uint8_t* img_buffer_device = nullptr;
	int img_size = img.total() * img.elemSize();
	CHECK(cudaMallocHost((void**)&img_buffer_host, img_size));
	CHECK(cudaMalloc((void**)&img_buffer_device, img_size));
    ...
    preprocess_kernel_t<<<blocks, threads, 0, stream>>>(img_buffer_device, device_input, src_w, src_h, dst_w, dst_h, d2s_matrix, jobs);
